...
//		}
		...
		{
			this.gameObject.GetComponent<Renderer>().material.SetFloat("_Alpha",1);
		}
		else if(fnowoffset>m_kShapeArc.GetCurveLength (0) - m_fHigh - linelenth && fnowoffset<=m_kShapeArc.GetCurveLength (0))
		{
			this.gameObject.GetComponent<Renderer>().material.SetFloat("_Alpha",1-((fnowoffset - m_kShapeArc.GetCurveLength (0) + m_fHigh + linelenth)/m_fHigh));
		}
		else 
		{
			this.gameObject.GetComponent<Renderer>().material.SetFloat("_Alpha",0);
